...
def train(args):
    ...
    for e in range(args.epoch):
        ...
        for i, batch in enumerate(tqdm(args.loader.train_iter, desc='train')):
            ...
            optimizer.zero_grad()
            args.model.train()
            x = args.model(feature)
            loss = criterion(x,feature)
            loss.backward()
            optimizer.step()
            ...
            if (i + 1) % args.printevery == 0:
                ...
                with torch.no_grad():
                    ...
                    for s, val_batch in enumerate(tqdm(args.loader.valid_iter, desc='valid')):
                        ...
                        loss = criterion(args.model(feature) , feature)
                        val_loss += loss.item()
...
def show_visual_progress(args, rows=5, title=None):
    ...
    for idx, (feature, label) in enumerate(args.loader.test_iter):
        ...
        images = args.model(feature).detach().cpu().numpy().reshape(feature.size(0), 28, 28)
...
def main():
    ...
    parser.add_argument('--experiment', type=str, default='Abnormal_class_0_ae', help='experiment name')
